...
def train(S,A,returns):
    ...
    # 計算 value function 的預測值
    V_pred = V(S).squeeze()
    
    # Value function 的損失：MSE between predicted values and actual returns
    ...
    for i in range(POLICY_TRAIN_ITERS):
        ...
        # 計算 advantage (使用 detach 避免 value function 的梯度影響 policy)
        with torch.no_grad():
            baseline = V(S).squeeze()
            advantages = returns - baseline
        
        # Policy gradient objective with baseline
        # 使用 advantage 代替 return
